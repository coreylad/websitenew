...
$Language = loadStaffLanguage('invite_gift');
$Message = '';
$amount = '0';
$usergroups = [];
$username = '';
            ...
            if ($username) {
                    ...
                    try {
                        ...
                        if ($stmt && $stmt->fetch(PDO::FETCH_ASSOC)) {
                            $newModComment = date('Y-m-d') . ' - ' . str_replace(
                                ...
                                $Language[13] ?? 'Invite gift of {2} to {1} by {3}'
                            ) . "\n";
                            
                            $TSDatabase->query(
                                'UPDATE users SET invites = invites + ?, 
                ...
                try {
                    ...
                    $SysMsg = str_replace(
                        ...
                        $Language[3] ?? 'Invite gift of {2} to groups {1} by {3}'
                    );
                    
                    $newModComment = date('Y-m-d') . ' - ' . $SysMsg . "\n";
                    
                    $query = 'UPDATE users SET invites = invites + ?, 
                              modcomment = IF(ISNULL(modcomment), ?, CONCAT(?, modcomment)) 
...
echo '
' . $Message . '
<form method="post" action="index.php?do=invite_gift">
' . getFormTokenField() . '
<table cellpadding="0" cellspacing="0" border="0" class="mainTable">
    <tr>
        <td class="tcat" colspan="2" align="center">' . ($Language[2] ?? 'Invite Gift') . '</td>
    </tr>
    <tr>
