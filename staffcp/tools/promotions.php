...
var_235();
$Language = file("languages/" . function_75() . "/promotions.lang");
...
if ($Act == "delete" && ($pid = intval($_GET["pid"]))) {
    ...
    function_79(str_replace("{1}", $_SESSION["ADMIN_USERNAME"], $Language[17]));
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (strtoupper($_SERVER["REQUEST_METHOD"]) == "POST") {
            ...
            if ($upload_limit || $ratio_limit || $min_reg_days || $posts || $times_warned) {
                ...
                function_79($Language[18]);
                $Message = function_76($Language[18]);
                $Updated = true;
            } else {
                $Message = function_76($Language[30]);
            }
        } else {
...
if (!isset($List)) {
    ...
    echo "\r\n\t<script type=\"text/javascript\">\r\n\t\tfunction ConfirmDelete(pID)\r\n\t\t{\r\n\t\t\tif (confirm(\"" . trim($Language[22]) . "\"))\r\n\t\t\t{\r\n\t\t\t\tTSJump(\"index.php?do=promotions&act=delete&pid=\"+pID);\r\n\t\t\t}\r\n\t\t\telse\r\n\t\t\t{\r\n\t\t\t\treturn false;\r\n\t\t\t}\r\n\t\t}\r\n\t</script>\r\n\t" . function_81("<span style=\"float: right;\"><a href=\"index.php?do=promotions&amp;act=new\">" . $Language[25] . "</a></span><a href=\"index.php?do=manage_cronjobs\">" . $Language[37] . "</a>") . "\r\n\t" . $Message . $List;
} else {
    echo $List;
}
