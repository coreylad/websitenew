...
var_235();
$Language = file("languages/" . function_75() . "/unconnectable_peers.lang");
$Message = "";
    ...
    if (strtoupper($_SERVER["REQUEST_METHOD"]) == "POST") {
        ...
        if ($userids[0] != "" && $ips[0] != "" && $ports[0] != "") {
            if (isset($_POST["check"])) {
                ...
                $Message = function_76($Language[22]);
            }
        }
    }
    $results = mysqli_num_rows(mysqli_query($GLOBALS["DatabaseConnect"], "SELECT * FROM peers WHERE $connectable = 'no'"));
    list($pagertop, $limit) = function_82(25, $results, $_SERVER["SCRIPT_NAME"] . "?do=unconnectable_peers&amp;");
