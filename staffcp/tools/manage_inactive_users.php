        ...
        if (mysqli_num_rows($query["warn"])) {
            ...
            while ($user = mysqli_fetch_array($query["warn"])) {
                if ($maxmails > $Totalwarn) {
                    ...
                    $sendmail = function_100($user["email"], $subject, nl2br($message));
                    if ($sendmail) {
            ...
            buildPaginationLinks("index.php?do=manage_inactive_users&amp;$show_per_page = " . $show_per_page . "&amp;$maxdays = " . $maxdays . "&amp;$deleteafter = " . $deleteafter . "&amp;$maxmails = " . $maxmails . "&amp;$waitlimit = " . $waitlimit . "&amp;$usergroups = " . $usergroups . "&amp;$act = warn", $waitlimit);
        } else {
...
{
    global $Language;
    echo "<br />" . formatTimestamp(str_replace("{1}", $seconds, $Language[39])) . "\r\n\t<meta http-$equiv = \"refresh\" $content = \"" . $seconds . ";$url = " . $url . "\" />";
    exit;
}
