...
function function_105($tzoffset = 0, $autodst = 0, $dst = 0)
{
    ...
    foreach (function_104() as $settingType => $settingDescription) {
        $settingValue .= "<option $value = \"" . $settingType . "\"" . ($tzoffset == $settingType ? " $selected = \"selected\"" : "") . ">" . $Language[$count] . "</option>";
        $count++;
    }
    $settingCategory = [];
    if ($autodst) {
...
function checkEditUserPermission($Option)
{
    ...
    return $currentUserPerms["edituserperms"][$settingHtml] == "1" ? true : false;
}
